...
Fifa <- subset(Fifa, select = -c(X, Name, Nationality, Club, Club_Position, Club_Joining, Birth_Date, Preffered_Foot, Preffered_Position, Work_Rate))
na.omit(Fifa)
dim(Fifa)
```
```{r display_orig_data}
...
```{r}
lmFit.many <- lm(Rating ~ ., data = Fifa)
summary(lmFit.many)
```
...
```{r}
Fifa_selected <- select(Fifa, Rating, Age, Skill_Moves, Ball_Control, Standing_Tackle, Reactions, Attacking_Position, Composure, Short_Pass, Speed, Strength, Heading, GK_Positioning, GK_Diving, GK_Kicking, GK_Handling, GK_Reflexes, Preffered_Position_encoded)
Fifa_selected
```
...
```{r}
fit.bs <- regsubsets(Rating ~ ., data = Fifa[1:2000,], nvmax = 47)
...
```
```{r, echo=FALSE}
fit.forward <- regsubsets(Rating ~ ., data = Fifa[1:nrow(Fifa)/2,], nvmax = 47, method = "forward")
fit.forward.summary <- summary(fit.forward)
fit.forward.summary
```
```{r}
fit.backward <- regsubsets(Rating ~ ., data = Fifa[1:nrow(Fifa)/2,], nvmax = 47, method = "backward")
...
```{r}
rating.tree <- tree(Rating ~ ., data = Fifa)
summary(rating.tree)
```
...
test <- -train
rating.tree <- tree(Rating ~ ., data = Fifa, subset = train)
rating.pred <- predict(rating.tree, newdata = Fifa[test,])
mean((rating.pred - Fifa$Rating[test])^2)
```
...
```{r}
rating.bag <- randomForest(Rating ~ ., data = Fifa[1:2000,], mtry = 10, importance = TRUE, na.action=na.roughfix)
rating.bag
```
```{r}
...
```{r}
set.seed(2)
rating.rf <- randomForest(Rating ~ ., data = Fifa[1:2000,], subset = train,
                         importance = TRUE, na.action=na.roughfix)
rating.pred.rf <- predict(rating.rf, newdata = Fifa[test,])
mean((rating.pred.rf - Fifa$Rating[test])^2)
```
...
```{r}
rating.boost <- gbm(Rating ~ ., data = Fifa[1:2000,], distribution = "gaussian",
...
Następnie przeprowadzimy walidację krzyżową wybranych trzech cech

```{r}
set.seed(2)
validation.set <- Fifa[-train,]
max.degree <- 11
mse <- rep(0, times = max.degree)
for (i in 1:max.degree) {
  fit.lm <- lm(Rating ~ poly(Reactions, degree = i), data = Fifa, subset = train)
...
```{r}
lmFitSimple <- lm(Rating ~ Reactions, data = Fifa)
summary(lmFitSimple)
```
```{r}
